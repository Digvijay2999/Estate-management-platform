...
export default function SellerPropertiesPage() {
  ...
  async function loadProperties() {
    try {
      const payload = await fetchJson('/seller/properties');
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map((property) => (
            <div key={property._id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-semibold text-slate-900">{property.title}</p>
                  <p className="mt-1 text-sm text-slate-600">Status: {property.status} • Assigned Agent: {property.agent?.fullName || 'Unassigned'}</p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => navigate(`/seller/properties/${property._id}/edit`)} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:border-slate-300">Edit</button>
